 ...
 * 
 * - Line 1: StaticPrefix ("WE") is permanently static and anchored.
 * - Line 2: The typewriter word cycles: words from HERO_CONTENT.
 * - Zero Layout Shift: An invisible sizing element containing the longest word
 ...
 * - Respects 'prefers-reduced-motion: reduce' by rendering static first word.
 */
export default function HeroHeadline({ className = '' }) {
  const words = HERO_CONTENT.words;
  const longestWord = HERO_CONTENT.longestWord;
  const staticPrefix = HERO_CONTENT.staticPrefix;
  ...
  if (prefersReducedMotion) {
    return (
      <h1 className={`font-display text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-extrabold tracking-tighter text-bbs-text uppercase leading-none m-0 ${className}`}>
        <span className="block text-bbs-text">{staticPrefix}</span>
        <span className="block text-bbs-accent-light">{words[0]}</span>
      </h1>
    );
  }
